...
class ActTwoTrialBase: SKScene {
    ...
    var instructionText: String {
        switch trial {
        case .mirrorHall: return "Найди отражение, которое отличается."
        case .candles: return "Погаси все свечи."
        case .whispers: return "Собери правильное слово."
        case .shadows: return "Найди лишнюю тень."
        case .clock: return "Заметь, что изменилось."
        case .rhyme: return "Восстанови пропущенную строку."
        case .lastDesk: return "Реши и подпиши тетрадь."
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        while let n = c {
            if n.name == "backButton" {
                back()
                return
            }
            if n.name == "action" || n.name == "task" {
                complete()
                return
            }
            c = n.parent
...
final class Trial8_MirrorHall: ActTwoTrialBase {
    init(size: CGSize) {
        super.init(size: size, trial: .mirrorHall)
    }
